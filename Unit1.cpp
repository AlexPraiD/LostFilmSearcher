...
//Выравнивание столбцов
	for(int i = 0; i < 5 ; i++) //9 - количество столбцов в гриде, изменяй эту велечину по мере добавления/удаления столбцов базы
		{
			if(i == 2) DBGrid1->Columns->Items[i]->Width = 200; //первый покороче
			else DBGrid1->Columns->Items[i]->Width = 30;  //остальные по 100
		}
}
//---------------------------------------------------------------------------
...
void __fastcall TForm1::Button1Click(TObject *Sender)
{
	...
	ProgressBar1->Max=ADOQuery1->RecordCount;
	 ...
	 Memo2->Lines->Add(" - Рабочие ссылки ["+IntToStr(k)+" из " +IntToStr(ADOQuery1->RecordCount)+"]");
}
//---------------------------------------------------------------------------
		...
		for(i=1;i<=ADOQuery1->RecordCount;i++)
			{
				...
				if( !TXT_Name->Text.Pos(Edit1->Text + " сезон " + Edit2->Text + " серия"))
					{
						Edit1->Text = (StrToInt(Edit1->Text) +1);
						Edit2->Text = 1;
					}

				while ( TXT_Name->Text.Pos(Edit1->Text + " сезон " + Edit2->Text + " серия"))
					{
					TXT_NameTwo->Text=TXT_Name->Text.Delete(1,TXT_Name->Text.Pos(Edit1->Text + " сезон " + Edit2->Text + " серия"));
					TXT_NameTwo->Text = TXT_NameTwo->Text.Delete(1,TXT_NameTwo->Text.Pos("4b4b4b")+7);
					Memo2->Lines->Add( Name + " " + Edit1->Text + " сезон " + Edit2->Text + " серия" + " [" + TXT_NameTwo->Text.SubString( 1,TXT_NameTwo->Text.Pos("</span>") -2 ) + "]" );

					ADOQuery2->Append();  //Добавление в историю
			...
			ProgressBar1->Visible=false;
	if(k==0)	Memo2->Text="                            ------------------ Ничего нового, Господин ------------------";
	if(k==1)	Memo2->Lines->Add("                            --------------- Итого: вышла "+IntToStr(k)+"  новая серия---------------");
	if( (k>=2) && (k<=4) )	Memo2->Lines->Add("                            --------------- Итого: вышли "+IntToStr(k)+"  новые серии---------------");
	if(k>2)	Memo2->Lines->Add("                            --------------- Итого: вышло "+IntToStr(k)+"  новых серий---------------");
}
//---------------------------------------------------------------------------
